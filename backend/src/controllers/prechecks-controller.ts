...
import { PRECHECK_CONFIG } from "../config/precheck-config";
...
export const runAllPrecheksHandler = async (req: Request, res: Response) => {
	...
	const precheckIds: string[] = PRECHECK_CONFIG.map((precheck) => precheck.id);
...
export const runPrechekByNodeIdHandler = async (req: Request, res: Response) => {
	...
	const selectedPrecheck = PRECHECK_CONFIG.filter((precheck) => precheckId === precheck.id);
	if (!selectedPrecheck) {
...
export const getPrecheckRunByClusterIdHandler = async (req: Request, res: Response) => {
	...
	const response = PRECHECK_CONFIG.map((precheck) => {
