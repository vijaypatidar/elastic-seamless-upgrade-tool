...
export const addLogs = async (clusterId: string, nodeId: string, timestamp: Date, message: string) => {
	const res = await Log.create({ clusterId, nodeId, timestamp, message });
	logger.debug(res);
};

export const getLogs = async (clusterId: string, nodeId: string, timestamp: Date | undefined): Promise<ILog[]> => {
	try {
